...
import {
  TColumn,
  TOrgCreditHistory,
  TTableOrgCreditHistory,
} from "@/types/organizationTypes";
import { Link } from "react-router-dom";
import { PrimaryBtn } from "@/components";

type TProps = {
  creditData?: TOrgCreditHistory[];
};
...
const CreditReportTab: React.FC<TProps> = ({ creditData }) => {
  const contactData: TTableOrgCreditHistory[] = creditData
    ? creditData.map((el) => ({
        type: el?.type,
        date: el?.date,
                ...
                >
                  {columns?.map((col: TColumn, i) => (
                    <td key={i} className="pl-4 font-light text-textGrey">
                      {row[col.field as keyof TTableOrgCreditHistory]}
                    </td>
                  ))}
                </tr>
              ))}
