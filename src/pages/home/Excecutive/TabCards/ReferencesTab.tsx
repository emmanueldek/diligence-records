import {
  TColumn,
  TOrgReferencesReputation,
  TTableOrgReferencesReputation,
...
const InsuranceCoverageTab: React.FC<TProps> = ({ data }) => {
  const contactData: TTableOrgReferencesReputation[] = data
    ? data.map((el) => ({
        type: el?.type,
                ...
                >
                  {columns?.map((col: TColumn, i) => (
                    <td key={i} className="pl-4 font-light text-textGrey">
                      {row[col.field as keyof TTableOrgReferencesReputation]}
                    </td>
                  ))}
                </tr>
              ))}
