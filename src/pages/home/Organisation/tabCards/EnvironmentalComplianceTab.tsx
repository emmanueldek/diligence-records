import {
  TColumn,
  TOrgEnvironmentalCompliance,
} from "@/types/organizationTypes";
import { PrimaryBtn } from "@/components";

type TProps = {
  data?: TOrgEnvironmentalCompliance[];
};

const columns: TColumn[] = [
  { field: "creditScore", header: "Credit Score" },
  { field: "reportingAgency", header: "Reporting Agency" },
  { field: "paymentHistory", header: "Payment History" },
  { field: "outstandingCreditLine", header: "    Outstanding Credit Line" },
];

const EnvironmentalComplianceTab: React.FC<TProps> = ({ data }) => {
  return (
    <div>
                ...
                >
                  {columns?.map((col: TColumn, i) => (
                    <td key={i} className="pl-4 font-light text-textGrey">
                      {row[col.field as keyof TOrgEnvironmentalCompliance]}
                    </td>
                  ))}
                </tr>
              ))}
...
export default EnvironmentalComplianceTab;
